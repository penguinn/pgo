...
// NewServerCodec returns a new core.ServerCodec using JSON-RPC 2.0 on conn,
// which will use srv to execute batch requests.
//
// If srv is nil then core.DefaultServer will be used.
//
...
// instead of core.DefaultServer to process requests on conn.
func NewServerCodec(conn io.ReadWriteCloser, srv *core.Server) core.ServerCodec {
	if srv == nil {
		srv = core.DefaultServer
	}
...
func NewServerCodecContext(ctx context.Context, conn io.ReadWriteCloser, srv *core.Server) core.ServerCodec {
...
func (c *serverCodec) ReadRequestHeader(ctx context.Context, r *core.Request) (err error) {
	// If return error:
...
func (c *serverCodec) ReadRequestBody(ctx context.Context, x interface{}) error {
...
func (c *serverCodec) WriteResponse(ctx context.Context, r *core.Response, x interface{}) error {
...
func ServeConn(conn io.ReadWriteCloser) {
	core.ServeCodec(NewServerCodec(conn, nil))
}
...
func ServeConnContext(ctx context.Context, conn io.ReadWriteCloser) {
	core.ServeCodec(NewServerCodecContext(ctx, conn, nil))
}
